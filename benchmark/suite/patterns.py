...
CONTEXT = "\n".join(
    ...
    for i, (adj, noun, n, topic) in enumerate(
        [(a, b, (7 * i + 13) % 40 + 1, t) for i, (a, b, t) in enumerate(
            [(a, b, t) for a in ("quarterly", "annual", "interim", "audited")
             for b in ("revenue", "safety", "latency", "quality")
             for t in ("cloud spend", "energy use")][:32])]
    )
)
...
# ============================ EASY: expected wins ============================
_p(name="map16", category="easy", n_expected_calls=16,
   hypothesis=("16 independent calls in a plain for-loop. The peek unrolls the "
...
_p(name="map_reduce", category="easy", n_expected_calls=9,
   hypothesis=("8-way map + dependent reduce. Map fans out during streaming; "
...
_p(name="batched8", category="easy", n_expected_calls=8,
   hypothesis=("llm_query_batched: the model already parallelized, so baseline "
...
_p(name="two_stage", category="easy", n_expected_calls=12,
   hypothesis=("Per-item chain (extract → judge) over 6 items. Stage-1 unrolls "
...
_p(name="late_loop", category="easy", n_expected_calls=8,
   hypothesis=("40 lines of pure compute BEFORE an 8-way map: dispatch can only "
...
_p(name="serial_chain6", category="hard", n_expected_calls=6,
   hypothesis=("6 calls, each consuming the previous output. Zero parallel "
               "width; speculation can only pre-dispatch call 1 during "
               "streaming. Expected floor: ≈1.0×, and MUST NOT be slower."),
   code=(
       "s = context.split('\\n')[0]\n"
       + "\n".join(f"s = llm_query('Shorten by one word: ' + str(s))" for _ in range(6)) + "\n"
       "answer['content'] = str(s)\nanswer['ready'] = True"),
   check=lambda ns: 1.0 if ns.get("s") else 0.0)

_p(name="branch_on_result", category="hard", n_expected_calls=3,
   hypothesis=("Control flow forced by a speculated value: the shadow must "
...
_p(name="mutating_loop", category="hard", n_expected_calls=6,
   hypothesis=("The loop mutates its own iterable through a subscript — the "
...
_p(name="early_break", category="hard", n_expected_calls=3,
   hypothesis=("Unroll overshoot: loop over 10 items breaks at i==2 on a "
...
_p(name="jail_break_mid", category="hard", n_expected_calls=6,
   hypothesis=("A non-whitelisted import (socket) lands after 3 calls: the "
   ...
   code=(
       ...
       "answer['ready'] = True"),
   check=lambda ns: 1.0 if str(ns.get("answer", {}).get("content", "")).startswith("6|socket") else 0.0)

_p(name="identical10", category="hard", n_expected_calls=10,
   hypothesis=("10 IDENTICAL prompts (self-consistency vote). Multiplicity "
               "rail: each must be an independent sample (FIFO), never one "
               "shared result. Check verifies output diversity under temp "
               "sampling; expected: full fan-out win + ≥2 distinct outputs."),
   code=(
       ...
       "answer['ready'] = True"),
   check=lambda ns: min(1.0, len(set(str(v).strip().lower() for v in ns.get("votes", []))) / 2)
   )

_p(name="crash_after_dispatch", category="hard", n_expected_calls=2,
   hypothesis=("IndexError right after 2 dispatches: real run fails identically "
...
# ============================ EDGE: curriculum ============================
_p(name="syntax_gauntlet", category="edge", n_expected_calls=4,
   hypothesis=("Calls hidden in a decorator'd helper, a dict comprehension, a "
...
_p(name="while_computed", category="edge", n_expected_calls=5,
   hypothesis=("While-loop with a computed bound (not peek-unrollable): the "
...
_p(name="multiblock", category="edge", n_expected_calls=6,
   hypothesis=("Two dependent blocks in one response: block 2 maps over "
               "variables computed in block 1. Shadow state must carry across "
               "block boundaries within the turn. Expected: normal win."),
   code=None,  # special-cased: full response text below
   check=lambda ns: _ans_lines(ns, "outs", 6))
PATTERNS[-1].code = ""  # placeholder; response built in runner for multiblock
MULTIBLOCK_RESPONSE = (
...
_p(name="prose_sandwich", category="edge", n_expected_calls=6,
   hypothesis=("Long prose BEFORE the block delays parsing; long prose AFTER "
...
def response_text(p: Pattern) -> str:
    if p.name == "multiblock":
        return MULTIBLOCK_RESPONSE
    head = PROSE_HEAD if p.name == "prose_sandwich" else "Working on it.\n"
