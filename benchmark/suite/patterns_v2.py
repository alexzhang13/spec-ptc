...
_p(name="map8_slow", category="easy", n_expected_calls=8, sub_max_tokens=320,
   hypothesis=("map16's twin at REALISTIC sub-call latencies (~5-sentence "
...
_p(name="chain4_slow", category="hard", n_expected_calls=4, sub_max_tokens=320,
   hypothesis=("Serial chain at realistic latencies — the honest floor where "
...
_p(name="map32", category="easy", n_expected_calls=32,
   hypothesis=("Width scaling: 32-way map probes the launcher's max_inflight "
...
_p(name="prefill_heavy", category="hard", n_expected_calls=6, sub_max_tokens=64,
   hypothesis=("6 calls each carrying the WHOLE context (~2.5k chars) — "
...
_p(name="no_peek_control", category="edge", n_expected_calls=8,
   hypothesis=("CONTROL for peek attribution: call args flow through a "
               "model-defined helper, which the pre-close peek does not "
               "resolve — all dispatches come from shadow execution at "
               "statement close. The delta vs a same-width plain map isolates "
               "the peek's marginal contribution."),
   code=(
       "def pick(i):\n"
...
_p(name="side_branch_chain", category="hard", n_expected_calls=8,
   hypothesis=("Mixed serial/parallel: a 4-step chain where each step ALSO "
