...
def _run_task(task: Task, model: str, base_url: str, spec: bool,
              log: TrajectoryLogger, outdir: Path,
              max_iterations: int = 12, max_timeout: float = 900.0) -> dict:
    tid = task.task_id
    env_kwargs = {}
    if spec:
        ...
        bus = EventBus()
        bus.record = False
        bus.subscribe(lambda ev: log.log("spec_" + ev.kind, task_id=tid,
                                         **{k: str(v)[:120] for k, v in ev.data.items()}))
    ...
    t0 = time.time()
    log.log("task_begin", task_id=tid, dataset=task.dataset, spec=spec,
            ctx_chars=len(task.context), question=task.question[:120])
    row = {"task_id": tid, "dataset": task.dataset, "spec": spec}
    try:
        out = r.completion(prompt=task.context,
                           root_prompt=f"{task.hint}\n\nQuestion: {task.question}")
        final = getattr(out, "response", "") or ""
        meta = getattr(out, "metadata", None)
        ts = _traj_stats(meta)
        sc = ts["subcall_times_s"]
        row.update(
            ok=True, score=score(task, str(final)), final_answer=str(final)[:300],
            rlm_execution_time_s=round(getattr(out, "execution_time", 0.0), 2),
            usage=str(getattr(out, "usage_summary", ""))[:300],
            n_turns=ts["n_turns"], n_subcalls=ts["n_subcalls"],
        ...
        row.update(ok=False, error=f"{type(e).__name__}: {e}",
                   trace=traceback.format_exc()[-800:], score=0.0)
    row["wall_s"] = round(time.time() - t0, 2)
    log.log("task_end", **row)
    return row


def run_unit(model: str, base_url: str, spec: bool, concurrency: int,
             repeat: int, tasks: list[Task], outdir: Path) -> dict:
    outdir = Path(outdir).resolve()
    log = TrajectoryLogger(outdir)
    _set_speculation(spec)
    log.log("unit_begin", model=model, spec=spec, concurrency=concurrency,
            repeat=repeat, n_tasks=len(tasks))
    t0 = time.time()
    try:
        with ThreadPoolExecutor(max_workers=concurrency) as pool:
            rows = list(pool.map(
                lambda t: _run_task(t, model, base_url, spec, log, outdir), tasks))
    finally:
        _set_speculation(False)
    unit_wall = time.time() - t0
    walls = [r["wall_s"] for r in rows]
    stats = {
        "model": model, "spec": spec, "concurrency": concurrency, "repeat": repeat,
