...
def summarize(df: pd.DataFrame) -> pd.DataFrame:
    g = (
        df.groupby(["model", "spec", "concurrency", "repeat"])
        .agg(
            ...
            turns=("n_turns", "mean"),
            subcalls=("n_subcalls", "mean"),
        )
        .reset_index()
    )
    out = (
        g.groupby(["model", "spec", "concurrency"])
        .agg(
...
def speedups(summary: pd.DataFrame) -> pd.DataFrame:
    piv = summary.pivot_table(
        index=["model", "concurrency"],
        columns="spec",
        values=["unit_wall_mean_s", "task_wall_mean_s"],
    )
    piv[("speedup", "unit")] = (
        piv[("unit_wall_mean_s", False)] / piv[("unit_wall_mean_s", True)]
    )
    piv[("speedup", "task")] = (
        piv[("task_wall_mean_s", False)] / piv[("task_wall_mean_s", True)]
    )
    return piv.round(2)


if __name__ == "__main__":
    base = Path(sys.argv[1] if len(sys.argv) > 1 else "benchmark/oolong_campaign/runs/main")
    df = load_units(base)
    s = summarize(df)
